...
class VIDEOCONTROLLER {
    async convertVideoWebmToMp4(originUrlVideo: string) {
        try {
            //const originUrlVideoFromLocal = `${__dirname}\input.webm`
            const { responseVideo } = await this.createVideoDocument(originUrlVideo)
            const {outputPath} = await videoprocesscontroller.convert(originUrlVideo, responseVideo?._id)
